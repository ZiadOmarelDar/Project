...
const Profile = () => {
  ...
  const handleClinicPhotosChange = async (e) => {
    ...
    if (validFiles.length !== files.length) {
      setErrors((prev) => ({
        ...prev,
        clinicPhotos:
          "Some files are not images or exceed 5MB. Only valid images will be added.",
      }));
    }

    const fileHashes = await Promise.all(
      validFiles.map((file) => getFileHash(file))
    );
    const uniqueFiles = validFiles.filter(
      (file, index) => fileHashes.indexOf(fileHashes[index]) === index
    );

    setFormData((prev) => {
      const updatedPhotos = [
        ...prev.clinicPhotos.filter((p) => typeof p === "object"),
        ...uniqueFiles,
      ].slice(0, 5);
  ...
  const handleRemovePhotoFromPreview = (indexToRemove) => {
    setFormData((prev) => {
      const newPhotos = prev.clinicPhotos.filter(
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:3001/user/upload-photo",
        formData,
        {
          headers: {
  ...
  const handleRemovePhoto = async () => {
    try {
      ...
      const response = await axios.delete(
        "http://localhost:3001/user/remove-photo",
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
      ...
      setUploadMessage(
        err.response?.data?.message || "Error removing photo. Please try again."
      );
    } finally {
      setLoading(false);
    }
  ...
  const validateForm = () => {
    ...
    if (!cleanedMobile.match(/^\+201[0-2,5]\d{8}$/)) {
      newErrors.mobile =
        "Please enter a valid Egyptian mobile number starting with +20 (e.g., +2010xxxxxxxx).";
    }
    ...
    if (
      ...
      formData.availablePrograms.length === 0
    ) {
      newErrors.availablePrograms =
        "Please select at least one available program.";
    }
    ...
    if (userData?.userType === "clinicAdmin") {
      ...
      if (!formData.location.specificLocation)
        newErrors.location = "Specific location is required.";
      if (!formData.workingHoursFrom || !formData.workingHoursTo)
        newErrors.workingHours = "Working hours are required.";
      if (!formData.servicePriceValue)
        newErrors.servicePriceValue = "Service price is required.";
      else if (
        isNaN(Number(formData.servicePriceValue)) ||
        Number(formData.servicePriceValue) < 0
      ) {
        newErrors.servicePriceValue =
          "Please enter a valid positive number for service price.";
      }
      if (!formData.serviceType)
        newErrors.serviceType = "Service type is required.";
      if (!formData.doctorDescription)
        newErrors.doctorDescription = "Doctor description is required.";
    }
  ...
  const handleServiceAction = async (e) => {
    ...
    formDataToSend.append(
      "type",
      userData.userType === "clinicAdmin" ? "clinic" : "trainer"
    );
    ...
    if (userData.userType === "clinicAdmin") {
      ...
      formDataToSend.append(
        "workingHours",
        `${formData.workingHoursFrom} ${formData.workingHoursFromPeriod} - ${formData.workingHoursTo} ${formData.workingHoursToPeriod}`
      );
    ...
    try {
      if (services.length > 0) {
        response = await axios.put(
          `http://localhost:3001/user/services/${services[0]._id}`,
          formDataToSend,
          {
            headers: {
              Authorization: `Bearer ${token}`,
    ...
    <div className="containerBox123">
      ...
      <div className="userProfileSection789">
        <div className="profileImageWrapper012">
          {userData.userPhoto && userData.userPhoto !== "not found" ? (
            <img
              src={`http://localhost:3001${userData.userPhoto}`}
              alt="Profile"
              className="profileImage345"
            />
          ) : (
        ...
        <div className="profileActions">
          ...
          <button className="btnPrimary">
            <FaUserCircle style={{ marginRight: "5px" }} /> Edit
          </button>
        </div>
      ...
      {(userData.userType === "trainer" ||
        ...
        <div className="servicesSection345">
          ...
          {errors.general && (
            <p className="errorMessage901">{errors.general}</p>
          )}
          <h3 className="servicesHeader234">Services Offered</h3>
          {services.length > 0 ? (
            services.map((service, index) => (
              <div className="customServiceCard">
                  ...
                  <div className="serviceDetailsGrid">
                    <div className="left-column">
                      ...
                      <p>
                        ...
                        {service.location.specificLocation}
                      </p>
                      <p>
                        <strong>Phone:</strong> {service.mobile}
                      </p>
                      <p>
                    ...
                    <div className="right-column">
                      ...
                      <p>
                        ...
                        {service.specialties.join(", ") || "Not provided"}
                      </p>
                      <div className="doctorDescriptionBox">
                        <p className="doctorDescriptionTitle">
                          Doctor Description:
                        </p>
                        <div className="doctorDescriptionContent">
                          {service.doctorDescription || "Not provided"}
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          {showAddServiceForm && (
            <div className="formContainer012">
              <form onSubmit={handleServiceAction} className="formContainer012">
                <h2 className="formHeader345">
                ...
                <div className="row">
                  <div className="clinicNameInput678">
                    ...
                    />
                    {errors.clinicName && (
                      <p className="errorDisplay901">{errors.clinicName}</p>
                    )}
                  </div>
                  <div className="doctorNameInput123">
                    ...
                    />
                    {errors.doctorName && (
                      <p className="errorDisplay456">{errors.doctorName}</p>
                    )}
                  </div>
                </div>
                <div className="row">
                  <div className="governorateSelect789">
                    ...
                    {errors.location && (
                      <p className="errorDisplay012">{errors.location}</p>
                    )}
                  </div>
                  <div className="specificLocationInput345">
                    ...
                    />
                    {errors.location && (
                      <p className="errorDisplay678">{errors.location}</p>
                    )}
                  </div>
                </div>
                <div className="row">
                  <div className="mobileInput901">
                    ...
                    />
                    {errors.mobile && (
                      <p className="errorDisplay123">{errors.mobile}</p>
                    )}
                  </div>
                  <div className="emailInput456">
                    ...
                    />
                    {errors.email && (
                      <p className="errorDisplay789">{errors.email}</p>
                    )}
                  </div>
                </div>
                <div className="row">
                  <div className="workingHoursInput345">
                    ...
                    {errors.workingHours && (
                      <p className="errorDisplay789">{errors.workingHours}</p>
                    )}
                  </div>
                  <div className="priceInput012">
                    ...
                    {errors.servicePriceValue && (
                      <p className="errorDisplay678">
                        {errors.servicePriceValue}
                      </p>
                    )}
                  </div>
                ...
                <div className="serviceTypeInput901">
                  ...
                  />
                  {errors.serviceType && (
                    <p className="errorDisplay123">{errors.serviceType}</p>
                  )}
                </div>
                ...
                <div className="row">
                  <div className="photoInput012">
                    <label>Upload Clinic Photos</label>
                    <button
                      type="button"
                      onClick={handleClinicPhotosChange}
                ...
                <div className="specialtiesInput012">
                  ...
                  {errors.specialties && (
                    <p className="errorDisplay678">{errors.specialties}</p>
                  )}
                </div>
                <div className="formButtons012">
                  <button type="submit" className="saveButton345">
                    Save
                  </button>
                  <button
                    type="button"
                    className="cancelButton678"
                    onClick={(e) => {
                      e.preventDefault();
                      setShowAddServiceForm(false);
                      setErrors({});
      ...
      <div className="updateLinkSection901">
        <a href="/profile/edit" className="updateProfileLink123">
          Update Profile
        </a>
      </div>
    </div>
